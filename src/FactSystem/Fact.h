...
class Fact : public QObject
{
    ...
    Q_PROPERTY(bool         quavRebootRequired      READ quavRebootRequired                                      CONSTANT)
...
public:
    ...
    /// Creates a Fact using the name and type from metaData. Also calls QUAVCorePlugin::adjustSettingsMetaData allowing
    ...
    QVariant rawValue() const { return m_rawValue; }  /// value prior to translation, careful
    ...
    FactMetaData::ValueType_t type() const { return m_type; }
    ...
    bool sendValueChangedSignals () const { return m_sendValueChangedSignals; }
    bool deferredValueChangeSignal() const { return m_deferredValueChangeSignal; }
    void clearDeferredValueChangeSignal() { m_deferredValueChangeSignal = false; }
    ...
    FactMetaData *metaData() { return m_metaData; }
...
protected:
    QString m_variantToString(const QVariant &variant, int decimalPlaces) const;
    void m_sendValueChangedSignal(const QVariant &value);

    QString m_name;
    int m_componentId = -1;
    QVariant m_rawValue = 0;
    FactMetaData::ValueType_t m_type = FactMetaData::valueTypeInt32;
    FactMetaData* m_metaData = nullptr;
    bool m_sendValueChangedSignals = true;
    bool m_deferredValueChangeSignal = false;

    static constexpr const char* kMissingMetadata = "Meta data pointer missing";

private slots:
    void m_checkForRebootMessaging();
