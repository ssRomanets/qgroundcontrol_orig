...
QString MapboxMapProvider::m_getURL(int x, int y, int zoom) const
{
    const QString mapBoxToken = SettingsManager::instance()->appSettings()->mapboxToken()->rawValue().toString();
    if (!mapBoxToken.isEmpty()) {
        if (m_mapTypeId == QStringLiteral("mapbox.custom")) {
        ...
        return MapBoxUrl.arg(m_mapTypeId).arg(zoom).arg(x).arg(y).arg(mapBoxToken);
    }
    return QString();
}
