...
QString CopernicusElevationProvider::m_getURL(int x, int y, int zoom) const
{
    Q_UNUSED(zoom)
    ...
    const QString url = m_mapUrl.arg(lat1).arg(lon1).arg(lat2).arg(lon2);
    return url;
}

QUAVTileSet CopernicusElevationProvider::getTileCount(int zoom, double topleftLon,
