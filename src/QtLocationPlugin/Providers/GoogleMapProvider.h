...
protected:
    GoogleMapProvider(const QString &mapName, const QString &versionRequest, const QString &version, const QString &imageFormat, quint32 averageSize,
        ...
        : MapProvider(
            ...
            averageSize,
            mapType)
        , m_versionRequest(versionRequest)
        , m_version(version) {}

private:
    void    m_getSecGoogleWords(int x, int y, QString& sec1, QString& sec2) const;
    QString m_getURL(int x, int y, int zoom) const final;

    const QString m_versionRequest;
    const QString m_version;
    const QString m_mapUrl = QStringLiteral("http://mt%1.google.com/vt/%2=%3&hl=%4&x=%5%6&y=%7&z=%8&s=%9&scale=%10");
    const QString m_secGoogleWord = QStringLiteral("Galileo");
    const QString m_scale = QStringLiteral("1");
};
