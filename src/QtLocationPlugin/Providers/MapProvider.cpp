...
int MapProvider::m_mapIdIndex = 1;
...
MapProvider::MapProvider(
    ...
    , m_mapStyle(mapStyle)
    , m_language(!QLocale::system().uiLanguages().isEmpty() ? QLocale::system().uiLanguages().constFirst() : "en")
    , m_mapId(m_mapIdIndex++)
{
    // qDebug() << Q_FUNC_INFO << this << _mapId;
}
...
QString MapProvider::m_tileXYToQuadKey(int tileX, int tileY, int levelOfDetail) const
{
    QString quadKey;
...
int MapProvider::m_getServerNum(int x, int y, int max) const
{
    return (x + 2 * y) % max;
}
...
QUAVTileSet MapProvider::getTileCount(int zoom, double topleftLon,
