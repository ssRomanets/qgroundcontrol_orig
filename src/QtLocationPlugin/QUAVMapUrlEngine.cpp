...
QUAVTileSet UrlFactory::getTileCount(int zoom, double topleftLon, double topleftLat, double bottomRightLon, double bottomRightLat, QStringView mapType)
{
    const SharedMapProvider provider = getMapProviderFromProviderType(mapType);
    if (provider) {
...
//    qWarning() << Q_FUNC_INFO << "map id not found:" << qtMapId;
    return QStringLiteral("");
}
...
//    qWarning() << Q_FUNC_INFO << "provider not found from id:" << qtMapId;
    return nullptr;
}
...
//    qWarning() << Q_FUNC_INFO << "type not found:" << type;
    return nullptr;
}
...
//    qWarning() << Q_FUNC_INFO << "provider not found from hash:" << hash;
    return QStringLiteral("");
}
