...
public:
    ...
    static QUAVTileSet getTileCount(int zoom, double topleftLon, double topleftLat,
                            double bottomRightLon, double bottomRightLat,
                            QStringView mapType);

    static const QList<std::shared_ptr<const MapProvider>>& getProviders() { return m_providers; }
...
private:
    static const QList<std::shared_ptr<const MapProvider>> m_providers;
};
