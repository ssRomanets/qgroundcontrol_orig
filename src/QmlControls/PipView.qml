...
Item {
    ...
    visible: item2 && item2.pipState !== item2.pipState.windowState && show

    property var    item1:                  null
    property var    item2:                  null
    property string item1IsFulalSettingsKey
    ...
    onItem1Changed: { m_initForItems() }

    onItem2Changed: { m_initForItems() }
    ...
    function m_initForItems() {  
        var item1IsFull = QUAV.loadBoolGlobalSetting("MainFlyWindowIsMap", true)
        if (item1 && item2) {
    ...
    function m_swapPip() {
        ...
        QUAV.saveBoolGlobalSetting("MainFlyWindowIsMap",item1IsFull)
    }
    ...
    Image {
        ...
        visible:            m_isExpanded && (ScreenTools.isMobile || pipMouseArea.containsMouse) &&
                            (
                                item2 !== null ?
                                (item2.pipState.state !== item2.pipState.initState && item2.pipState.state !== item2.pipState.windowState) : false
                            ) &&
                            (
                                item1 !== null ? (item1.pipState.state !== item1.pipState.windowState) : false
                            )
    ...
    Timer {
        ...
        onTriggered: {
            if (globals.cameraStreamAct)
            {
                if (item2 !== null)
                {
                    if (item2.pipState.state !== item2.pipState.initState && !m_videoManager.isUvc)
                    {
                        if (item2.pipState.state === item2.pipState.windowState || item1.pipState.state === item1.pipState.windowState)
                        {
                            item2.pipState.state = item2.pipState.initState

                            if (item1.pipState.state === item1.pipState.windowState)
                                item1.pipState.state = item1.pipState.fullState

                            window.close()
                        }
                        else item2.pipState.state = item2.pipState.initState

                        if (item1.pipState.state === item1.pipState.pipState) item1.pipState.state = item1.pipState.fullState
                    }
                    else if ( (item2.pipState.state === item2.pipState.initState) && m_videoManager.isUvc)
                    {
                        if (globals.videoOnWindow) {
                            item1.pipState.state = item1.pipState.pipState
                            item2.pipState.state = item2.pipState.fullState
                        } else {
                            item1.pipState.state = item1.pipState.fullState
